...
const taskStatusMeta = {
  ...
  "中止中": { color: "orange", actions: ["detail"] },
...
const taskListRows = [
  ...
  { id: "OTA202605080001", name: "硅谷摄像机批量灰度", method: "指定版本", quantityMode: "batch", planned: 800, batchLimit: 200, maxDispatchTotal: 800, packageType: "整包", targetVersion: "23.422.209.17", total: null, time: "2026-05-08 10:00:00~2026-05-18 10:00:00", result: { matched: 640, success: 520, failed: 20, running: 100 }, region: "硅谷", dispatchRegions: ["硅谷"], status: "中止中", creator: "江锐", createdAt: "2026-05-08 09:42:16" },
];

const taskStatusOptions = ["全部", "待发布", "待审批", "待执行", "升级中", "中止中", "已完成", "已结束", "已驳回", "已失效"];
...
const roles = [
  ...
  { name: "产线主管", count: 7, note: "负责审批升级任务，关注风险评估和产线覆盖范围" },
...
function dispatchQuantitySummary(source = state) {
  ...
  return `每个区域每个版本单批数 ${Number(batchLimit || 0).toLocaleString()} 台 · 任务累计下发上限 ${maxTotal ? `${Number(maxTotal).toLocaleString()} 台` : "不限制"}`;
}
...
function renderRequirementDock() {
  return `
    <div class="requirement-dock-group" aria-label="需求批注开关">
      <button class="requirement-dock ${state.showRequirementNotes ? "active" : ""}" type="button" data-action="toggle-requirement-notes" aria-pressed="${state.showRequirementNotes}">
        ${icon("info")}
        <span>${state.showRequirementNotes ? "隐藏批注" : "显示批注"}</span>
      </button>
    </div>
  `;
}
  ...
  return `
    <section class="page requirements-page prd-doc-shell">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("PRD DOC", "需求说明页入口", [
          "该页面用于集中展示 OTA 升级任务管理优化 PRD。",
          "内容按章节卡片拆分，便于产品评审、研发实现和测试验收逐段对照。",
          "返回任务列表不改变当前原型状态，仅切换路由。",
        ])}
      </div>
      <section class="prd-deck-layout">
        <div class="annotated-block">
          ...
          ${renderRequirementNote("PRD DOC", "章节导航", [
            "左侧导航按 PRD 二级章节生成，点击后定位到对应章节卡片。",
            "导航标题来自内置 PRD Markdown，需与文档章节保持一致。",
            "页面用于评审阅读，不参与任务创建或发布数据提交。",
          ])}
        </div>
        <div class="prd-slide-stack">
          ${slides.map((slide, index) => `
            <div class="annotated-block">
              ...
              ${index === 0 ? renderRequirementNote("PRD DOC", "章节内容卡片", [
                "每张卡片承载一个 PRD 章节，保留表格、列表和流程图。",
                "Markdown 图片路径会自动解析到 docs/assets，确保静态部署后仍可访问。",
                "内容来源为页面内置 requirementsMarkdown，与文档正文需同步维护。",
              ]) : ""}
            </div>
          `).join("")}
        </div>
...
function renderRequirementNote(section, title, items) {
  ...
  return `
    <button class="requirement-pin" type="button" aria-label="${escapeHtml(section)} ${escapeHtml(title)} 批注">
      <i>!</i>
      <span class="requirement-popover" role="tooltip">
        <span class="requirement-popover-head">
          <em>${escapeHtml(section)}</em>
    ...
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("PRD 01 / 11", "本次范围与状态统计", [
          "原型覆盖新增任务、任务列表、任务详情、升级统计规则、异常分类和模拟验证。",
      ...
      <div class="annotated-block">
        ...
        ${renderRequirementNote("PRD 11", "状态统计卡片", [
          "展示全部任务和基础状态数量。",
          "待发布、待审批、升级中、中止中、已完成等状态需与列表筛选口径一致。",
        ])}
      </div>
      <div class="annotated-block">
        ${renderTaskFilters()}
        ${renderRequirementNote("PRD 11", "列表查询条件", [
          "查询条件顺序：任务名称/目标版本号、任务状态、升级方式、升级包、创建人、创建时间。",
          "任务名称输入框同时支持任务名称和目标版本号模糊查询，例如输入 23.422 可定位同一目标版本任务。",
      ...
      <div class="annotated-block">
        ...
        ${renderRequirementNote("PRD 11", "列表字段与状态操作", [
            ...
            "操作规则：待发布编辑/删除，升级中详情/结束任务，中止中仅详情，已驳回和已失效详情/复制重建。",
    ...
    <div class="task-filter-panel">
      <div class="task-filter-grid">
        <label class="filter-field filter-keyword">
          <span>任务名称/目标版本号</span>
          <span class="field-control task-search">
            ${icon("search")}
            <input type="search" value="${escapeHtml(filters.keyword)}" placeholder="请输入任务名称或目标版本号" aria-label="任务名称或目标版本号" data-task-filter="keyword" />
          </span>
        </label>
...
function resetDetailViewContext(options = {}) {
  ...
  state.deviceExportMenuOpen = false;
...
function publishedTaskConditionText() {
  ...
  if (state.strategy === "version") {
    return `升级区域 = ${dispatchRegionSummary(state.dispatchRegions)}；${dispatchQuantitySummary()}${customCondition !== "未配置" ? `；策略条件：${customCondition}` : ""}`;
  }
...
function publishedEndResultForTask(task) {
  if (task.method === "指定版本") {
    ...
    const running = Math.min(Math.max(Math.round(matched * 0.18), 1), matched);
    return {
      matched,
      success: Math.max(matched - running - 8, 0),
      failed: Math.min(8, matched),
      running,
    };
  }
  const total = Number(task.total || 0);
  const running = total > 1 ? 1 : 0;
  const failed = total ? 1 : 0;
  const success = Math.max(total - running - failed, 0);
...
function taskMatchesFilters(task) {
  ...
  if (keyword) {
    const nameMatched = normalizeText(task.name).includes(keyword);
    const targetVersionMatched = normalizeText(task.targetVersion).includes(keyword);
    if (!nameMatched && !targetVersionMatched) return false;
  }
  ...
  return `
    <div class="cascader-dropdown scope-region-dropdown" data-stop>
      <div class="select-dropdown-tools">
        <span>${isCondition ? "设备所属大区条件" : "升级区域"}</span>
        <div>
  ...
  return `
    <section class="workbench-card step-card">
      ...
      ${renderRequirementNote("PRD 06 / 07 / 08", "策略配置规则", [
        ...
        "全量展示全量；批量支持每个区域每个版本单批数和任务累计下发上限。",
...
function renderVersionStrategy(packageLabel) {
  ...
  return `
    ...
    <div class="rule-panel">
      ...
      ${renderVersionDispatchRegionPicker()}
      <div class="quantity-panel ${state.quantityMode === "batch" ? "is-batch" : "is-full"}">
        <div class="quantity-panel-head">
          ...
          <div class="segmented compact-segmented" role="radiogroup" aria-label="升级数量">
            ...
            ${radio("quantityMode", "batch", "批量下发", state.quantityMode)}
          </div>
        </div>
        ${state.quantityMode === "batch"
          ? `
            <div class="batch-rule-grid">
              <label class="batch-rule-item ${state.errors.batchLimit ? "has-error" : ""}">
                <span class="batch-rule-title">每个区域每个版本单批数</span>
                <span class="batch-rule-field">
                  <input class="table-input" type="number" min="0" value="${state.batchLimit}" aria-label="每个区域每个版本单批数" data-batch-limit />
                  <em>台</em>
                </span>
                <span class="batch-rule-help">系统会分别对每个已选区域、每个源版本按该数量下发。</span>
                ${renderFieldError("batchLimit")}
              </label>
              <label class="batch-rule-item max-total ${state.errors.maxDispatchTotal ? "has-error" : ""}">
                <span class="batch-rule-title">任务累计下发上限</span>
                <span class="batch-rule-field">
                  <input class="table-input" type="number" min="0" value="${state.maxDispatchTotal}" placeholder="不限制" aria-label="任务累计下发上限" data-max-dispatch-total />
                  <em>台</em>
                </span>
                <span class="batch-rule-help">留空表示不限制；整个任务累计下发达到该数量后，将停止匹配并不再下发新设备。</span>
                ${renderFieldError("maxDispatchTotal")}
              </label>
            </div>
          `
          : `<p class="quantity-mode-note">按已选区域和源版本匹配全部可升级设备。</p>`}
      </div>
  ...
  return `
    <section class="sync-region-panel dispatch-region-panel">
      <div class="sync-region-head">
        <span><span class="required">*</span> 升级区域</span>
        <em>已选择 ${selected.length} 个区域</em>
      </div>
    ...
    <section class="page">
      ...
      <div class="annotated-block">
        ...
        ${renderRequirementNote("PRD 12", "顶部任务概览卡", [
          ...
          "展示任务标识、创建人、更新时间、任务说明、目标版本、任务时间、升级方式、升级包和升级区域。",
      ...
      <div class="annotated-block compact-note-block">
        ...
        ${renderRequirementNote("PRD 12", "详情页信息架构", [
          "主页签固定为任务概览和升级明细。",
          "任务概览展示任务配置和流转；升级明细先展示区域升级明细，再展示当前区域的升级概览、异常分类和设备列表。",
          "开发评审视图可切换状态和统计规则，但不影响真实任务记录字段。",
        ])}
      </div>
  ...
  if (state.detailTab === "devices") {
    ...
    return `
        ...
        <div class="annotated-block">
          ...
          ${renderRequirementNote("PRD 12", "升级概览统计规则", [
            "文件/手动导入展示固定设备总数、已处理、成功、失败、未处理。",
            "指定版本全量展示已匹配数、成功数、失败数、升级中，不展示未知总数百分比。",
            "指定版本批量展示每个区域每个版本单批数、任务累计下发上限、已匹配数、成功数、失败数和升级中。",
  ...
  return `
    <div class="annotated-block">
      ...
      ${renderRequirementNote("PRD 12 / 09", "任务进度与流转明细", [
        ...
        "中止中表示用户已确认结束但仍等待升级中设备回执，期间不再允许重复结束任务。",
        "已完成包含成功和失败设备；已结束表示用户提前手动结束后的最终收敛状态，需记录结束时间和结束原因。",
      ])}
    </div>
  `;
}
...
function isDetailExecutionStatus(detail) {
  return ["升级中", "中止中", "已完成", "已结束"].includes(detail.status);
}
...
function detailStatusOptionsForMode() {
  return isManualDetailMode()
    ? ["待执行", "升级中", "中止中", "已完成", "已结束"]
    : ["待审批", "已驳回", "已失效", "待执行", "升级中", "中止中", "已完成", "已结束"];
}
...
function taskDetailData(status, task = null) {
  ...
  const variants = {
    ...
    "中止中": { success: 530, failed: 14, running: 628, pending: 5333, stoppingAt: "2026-06-11 15:12:30", stopReason: "用户已确认结束任务，系统停止新增匹配和新增下发，等待已下发设备回执收敛" },
...
function detailMetricsFromTaskStatus(status, task, base) {
  ...
  if (!["升级中", "中止中", "已完成", "已结束"].includes(status)) {
...
function normalizeDetailMetrics(detail) {
  if (isManualDetailMode()) {
    ...
    const manualMetrics = {
      ...
      "中止中": { success: Math.max(total - 1, 0), failed: 0, running: total > 1 ? 1 : 0, pending: 0 },
  ...
  if (isVersionBatchDetailMode()) {
    const batchMetrics = {
      ...
      "中止中": { success: 2380, failed: 24, running: 196, pending: 2400 },
...
function renderDetailStatusSwitch() {
  ...
  return `
    <div class="detail-status-switch">
      ...
      <div class="simulation-row">
        <span>${selectedTask ? "当前数据" : "升级方式"}</span>
        <div>
  ...
  return `
    <section class="region-detail-panel">
      <div class="region-detail-head">
        <div>
          <h3 class="section-title">${icon("map")}区域升级明细</h3>
        </div>
        <span>${rows.length} 个区域</span>
      </div>
      <div class="region-tab-strip" role="tablist" aria-label="升级区域">
        ${rows.map(row => `
          <button class="${regionTabKey(row.region) === activeKey ? "active" : ""}" type="button" role="tab" aria-selected="${regionTabKey(row.region) === activeKey}" data-action="set-detail-region-tab" data-region-tab="${escapeHtml(regionTabKey(row.region))}">
            ...
            <span>${Number(row.matched).toLocaleString()} 台 · ${row.status}</span>
            <em>成功率 ${row.successRate}%</em>
          </button>
        `).join("")}
      </div>
      <div class="region-current-strip">
        <div>
          <span>当前查看</span>
...
function executionProgressText(detail, stats) {
  ...
  if (isVersionBatchDetailMode() && !plannedBatchTotal()) {
    return `当前已处理 ${Number(stats.stocked).toLocaleString()} 台，任务累计下发上限不限制`;
  }
  ...
  if (isVersionFullDetailMode() || isVersionBatchDetailMode()) {
    fields.push(
      ...
      },
      {
        label: "升级对象",
        icon: "map",
        value: `${dispatchRegionSummary(detail.dispatchRegions)}；指定源版本设备。`,
        span: "full",
      },
    );
  } else {
...
function taskFlowSummary(detail) {
  const map = {
    ...
    "中止中": {
      node: "中止中",
      text: "用户已确认结束任务，系统已停止新增匹配和新增下发，正在等待已下发设备回执或超时收敛。",
      extra: `中止时间：${detail.stoppingAt || "-"}`,
      tone: "orange",
    },
    "已完成": {
      node: "任务完成",
...
function detailExecutionSummary(detail) {
  ...
  if (isVersionFullDetailMode()) {
    ...
    if (detail.status === "中止中") return `已匹配 ${Number(stats.stocked).toLocaleString()} 台，已停止新增匹配，等待 ${Number(stats.running).toLocaleString()} 台升级中设备回执`;
  ...
  if (isVersionBatchDetailMode()) {
    ...
    if (detail.status === "中止中") return `已匹配 ${Number(stats.stocked).toLocaleString()} / ${planText}，已停止新增下发，等待 ${Number(stats.running).toLocaleString()} 台设备回执`;
  ...
  if (detail.status === "中止中") {
    return `执行进度：已处理 ${Number(stats.stocked).toLocaleString()} / ${Number(detail.total).toLocaleString()} 台，正在等待 ${Number(stats.running).toLocaleString()} 台升级中设备回执`;
  }
...
function taskFlowRecords(detail) {
  ...
  } else if (["升级中", "中止中", "已完成", "已结束"].includes(detail.status)) {
    records.push({ title: "开始下发", operator: "系统", time: detail.startAt, desc: "到达任务开始时间，开始下发 OTA 指令", type: "done" });
  }
  if (["升级中", "中止中", "已完成", "已结束"].includes(detail.status)) {
    if (detail.status === "中止中") {
      records.push({ title: "用户申请结束", operator: detail.creator, time: detail.stoppingAt || "-", desc: detail.stopReason || "系统停止新增匹配和新增下发，等待已下发设备回执", type: "active" });
    } else {
...
function executionRecordText(detail) {
  ...
  if (isVersionBatchDetailMode() && !denominator) return `已处理 ${Number(stats.stocked).toLocaleString()} 台，任务累计下发上限不限制`;
  return `已处理 ${Number(stats.stocked).toLocaleString()} / ${Number(denominator).toLocaleString()} 台`;
}

function taskFlowNodes(detail) {
  const approvalDone = ["待执行", "升级中", "中止中", "已完成", "已结束"].includes(detail.status);
  const executionStarted = ["升级中", "中止中", "已完成", "已结束"].includes(detail.status);
  ...
  } else if (detail.status === "中止中") {
    nodes[4] = { ...nodes[4], desc: "已停止新增下发，等待已下发设备回执", type: "active" };
...
function taskApprovalNodeTime(detail) {
  if (["已驳回", "已失效", "待执行", "升级中", "中止中", "已完成", "已结束"].includes(detail.status)) {
...
function taskFinishNodeTime(detail) {
  if (detail.status === "中止中") return detail.stoppingAt || "-";
...
function renderExecutionOverview(detail) {
  ...
  if (!isDetailExecutionStatus(detail)) {
    const label = isVersionFullDetailMode() ? "升级规模" : isVersionBatchDetailMode() ? "任务累计下发上限" : "升级设备总数";
...
function renderUpgradeDetailOverview(detail) {
  const currentRegion = detail.method === "指定版本" ? selectedDetailRegionName(detail) : "";
  const title = currentRegion ? `${compactRegionName(currentRegion)}升级进度` : "升级概览";
  const subtitle = currentRegion ? `当前区域：${currentRegion}` : "";
  return `
    <div class="upgrade-detail-overview">
      <div class="section-title-row upgrade-overview-title-row">
        <h3 class="section-title">${icon("layer")}${title}</h3>
...
function renderExecutionProgressInline(detail, stats) {
  const description = isVersionFullDetailMode()
    ...
    : isVersionBatchDetailMode()
      ? (plannedBatchTotal() ? `已处理 ${Number(stats.stocked).toLocaleString()} / 任务累计下发上限 ${Number(plannedBatchTotal()).toLocaleString()} 台` : `已处理 ${Number(stats.stocked).toLocaleString()} 台，任务累计下发上限不限制`)
      : `已处理 ${Number(stats.stocked).toLocaleString()} / ${Number(detail.total).toLocaleString()} 台`;
  const resultText = detail.status === "已结束"
    ? "任务已提前结束，未处理设备不再继续下发"
    : detail.status === "中止中"
      ? "中止中，已停止新增下发，等待升级中设备回执"
      : detail.status === "已完成"
        ? "任务已完成，设备结果已归档"
        : "执行中，设备结果持续回传";
  return `
      ...
      <div class="exception-donut-wrap">
        <div class="exception-echart">
          ...
          <div class="exception-donut-center">
            <span data-exception-center-label data-default="${detail.method === "指定版本" ? "当前区域异常" : "异常总数"}">${detail.method === "指定版本" ? "当前区域异常" : "异常总数"}</span>
            <strong data-exception-center-value data-default="${Number(total).toLocaleString()} 台">${Number(total).toLocaleString()} 台</strong>
            <em data-exception-center-percent data-default="${detail.method === "指定版本" ? "当前区域" : "异常占比"}">${detail.method === "指定版本" ? "当前区域" : "异常占比"}</em>
          </div>
        </div>
      </div>
...
function renderDeviceDetailTable(detail) {
  ...
  const rows = filteredRows.length
    ...
    : `<tr class="empty-row"><td colspan="8">${selectedRegion ? "当前区域暂无符合条件的设备" : "暂无符合条件的设备"}</td></tr>`;
  return `
    <div class="section-title-row">
      <h3 class="section-title">${icon("layer")}设备列表</h3>
      ${detail.method === "指定版本" ? `<span class="section-subtitle">当前区域：${escapeHtml(selectedRegion)}</span>` : ""}
    </div>
    ...
    <div class="toolbar device-list-toolbar">
      ...
      <div class="export-menu-wrap" data-stop>
        <button class="btn export-menu-trigger ${state.deviceExportMenuOpen ? "active" : ""}" type="button" data-action="toggle-device-export-menu" aria-expanded="${state.deviceExportMenuOpen}" aria-haspopup="menu">
          ${icon("download")}导出设备明细 ${icon("chevron")}
        </button>
        ${state.deviceExportMenuOpen ? renderDeviceExportMenu(detail, selectedRegion) : ""}
      </div>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>设备ID</th><th>源版本</th><th>目标版本</th><th>所属大区</th><th>升级状态</th><th>完成时间</th><th>失败原因</th><th>最近上报时间</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
  `;
...
function renderDeviceExportMenu(detail, selectedRegion) {
  const currentScope = selectedRegion ? `当前区域：${selectedRegion}` : "当前筛选结果";
  const allScope = detail.method === "指定版本" ? "全部升级区域、源版本和批次" : "全部设备执行记录";
  return `
    <div class="export-menu" role="menu">
      <button type="button" role="menuitem" data-action="export-device-list" data-export-scope="current">
        <strong>导出当前筛选结果</strong>
        <span>${escapeHtml(currentScope)}，包含当前搜索条件</span>
      </button>
      <button type="button" role="menuitem" data-action="export-device-list" data-export-scope="all">
        <strong>导出全部设备明细</strong>
        <span>${escapeHtml(allScope)}，生成 ZIP 后由浏览器下载</span>
      </button>
    </div>
  `;
}
...
function renderDeviceDetailNote(detail) {
  const fixed = detail.method !== "指定版本";
  return `
    <div class="device-detail-note">
      <strong>${fixed ? "固定清单设备明细" : "指定版本设备明细"}</strong>
      <p>${fixed ? "文件导入和手动导入按设备所属大区拆分，下表展示当前清单设备的执行结果。" : "指定版本按当前选中的分发范围展示设备，列表不提供跨区域全部聚合明细。"}</p>
    </div>
  `;
}
...
function buildDeviceDetailRow(detail, id, index, forcedRegion = "", sampleMetric = null) {
  ...
  const waiting = ["升级中", "中止中"].includes(detail.status) && pending;
...
function renderDeviceDetailRow(row) {
  return `<tr><td>${row.id}</td><td>${row.sourceVersion}</td><td>${row.targetVersion}</td><td>${row.region}</td><td>${row.upgradeStatus}</td><td>${row.finishedAt}</td><td>${row.reason}</td><td>${row.reportedAt}</td></tr>`;
}
...
function deviceDetailSampleCounts(detail, sampleMetric = null) {
  ...
  let runningCount = ["升级中", "中止中"].includes(detail.status) ? Math.min(runningSource, 3, remaining) : 0;
  remaining -= runningCount;
  let successCount = Math.min(successSource, remaining);
  remaining -= successCount;
  if (["升级中", "中止中"].includes(detail.status) && runningSource > runningCount && remaining > 0) {
...
function versionDeviceSampleLimit(detail, regionMetric) {
  ...
  const runningCount = ["升级中", "中止中"].includes(detail.status) ? Math.min(Number(regionMetric.running) || 0, 3, remaining) : 0;
  remaining -= runningCount;
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote(title === "操作日志" ? "LOG 01" : "LOG 02", `${title}页面目标`, [
          title === "操作日志" ? "记录用户在任务、用户、角色等模块中的关键操作。" : "记录 OTA 任务提交审批、审批通过、审批驳回等审批链路事件。",
          "日志页面用于审计追溯，不提供编辑或删除入口。",
          "列表默认按时间倒序展示，分页保持与查询条件一致。",
        ])}
      </div>
      <div class="annotated-block">
        <div class="toolbar">
        ...
        ${renderRequirementNote(title === "操作日志" ? "LOG 01" : "LOG 02", "日志查询条件", [
          "支持按开始时间和结束时间筛选日志。",
          "查询后保留当前日志类型，重置后清空筛选条件。",
          "时间字段用于审计定位，应精确到秒或至少精确到分钟。",
        ])}
      </div>
      <div class="annotated-block">
        <div class="table-wrap">
        ...
        ${renderRequirementNote(title === "操作日志" ? "LOG 01" : "LOG 02", "日志列表字段", [
          "操作时间、操作人、操作模块、操作动作、事件描述均为审计必看字段。",
          "长文本通过 title 保留完整内容，表格中保持单行扫描效率。",
          "操作日志与审批日志分开展示，避免审批流和普通操作流混淆。",
        ])}
      </div>
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("LOG 02", "审批日志页面目标", [
          "审批日志用于追溯 OTA 任务从提交到审批完成的流转过程。",
          "审批记录应能关联任务名称、操作人、审批动作和任务状态。",
          "页面只做查询审计，不改变任务状态。",
        ])}
      </div>
      <div class="annotated-block">
        ...
        ${renderRequirementNote("LOG 02", "审批日志查询", [
          "按任务名称检索审批记录，便于定位单个 OTA 任务审批过程。",
          "查询和重置动作不触发任务状态变更。",
          "后续可扩展审批人、审批动作和时间范围筛选。",
        ])}
      </div>
      <div class="annotated-block">
        <div class="table-wrap">
        ...
        ${renderRequirementNote("LOG 02", "审批日志列表字段", [
          "审批时间用于排序和审计定位。",
          "审批动作需区分提交审批、审批通过、审批驳回等节点。",
          "任务状态展示审批动作后的状态结果，审批描述补充流转说明。",
        ])}
      </div>
      ${pagination(12, 1)}
    </section>
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("USER 01", "用户列表页面目标", [
          "用户列表用于查看账号、所属产线、关联角色和创建时间。",
          "页面提供用户详情和权限配置入口，不在列表中直接修改角色。",
          "用户角色权限控制功能操作，不承担 OTA 任务大区过滤职责。",
        ])}
      </div>
      <div class="annotated-block">
        <div class="toolbar">
        ...
        ${renderRequirementNote("USER 01", "用户查询条件", [
          "支持按用户账号名称搜索，便于快速定位目标人员。",
          "查询结果与分页联动，重置后恢复默认列表。",
          "当前原型聚焦功能权限，不展示组织树或大区授权筛选。",
        ])}
      </div>
      <div class="annotated-block">
        <div class="table-wrap">
        ...
        ${renderRequirementNote("USER 01", "用户列表字段与操作", [
          "账号名称、关联角色、所属产线和创建时间为基础管理字段。",
          "详情用于查看账号与角色关系；权限配置用于调整该用户功能权限。",
          "操作入口需受当前登录人权限控制，本原型默认系统管理员可见。",
        ])}
      </div>
      ${pagination(80, 4)}
    </section>
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("USER 02", "用户权限配置入口", [
          "从用户列表进入，配置对象为单个用户。",
          "返回按钮回到用户列表，取消不保存变更。",
          "权限配置只影响功能操作入口，不改变任务数据口径或设备分发规则。",
        ])}
      </div>
      <div class="annotated-block">
        ...
        ${renderRequirementNote("USER 02", "用户权限矩阵", [
          "权限按菜单、页面和功能操作三级展示。",
          "OTA 升级权限包括查看列表、查看详情、新增、编辑、发布、中止和删除任务。",
          "保存后应刷新用户权限；取消返回列表且不保存勾选变更。",
        ])}
      </div>
    </section>
  `;
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote("USER 03", "用户详情页面目标", [
          "用户详情用于查看账号基础信息和关联角色。",
          "详情页只读展示，不提供直接编辑账号资料。",
          "返回后保留用户列表作为主入口。",
        ])}
      ...
      <div class="annotated-block">
        ...
        ${renderRequirementNote("USER 03", "用户关联角色", [
          "关联角色决定该用户默认拥有的功能权限集合。",
          "角色名称、关联用户数和备注用于判断角色职责边界。",
          "本页不展示 OTA 任务数据范围授权，避免与任务分发大区混淆。",
        ])}
      </div>
    </section>
  `;
  ...
  return `
    <section class="page">
      <div class="annotated-block">
        ...
        ${renderRequirementNote(forcePermission ? "ROLE 02" : "ROLE 01", forcePermission ? "角色权限配置入口" : "角色权限页面目标", [
          forcePermission ? "配置权限页面用于编辑当前角色的功能权限勾选项。" : "角色权限页面用于管理系统角色、查看角色权限和关联用户。",
          "角色权限控制用户可见和可操作的功能入口。",
          "角色权限不改变 OTA 任务的跨大区分发规则，只影响是否能创建、发布、中止等操作。",
        ])}
      </div>
      <div class="annotated-block">
        ...
        ${renderRequirementNote(forcePermission ? "ROLE 02" : "ROLE 01", "角色列表与权限内容", [
          "左侧角色列表用于切换当前查看或配置的角色。",
          "角色权限页签展示菜单、页面和功能操作权限；关联用户页签展示已绑定用户。",
          forcePermission ? "保存权限后返回角色列表；取消不保存本次修改。" : "默认角色不允许删除，新增角色通过弹窗录入名称和备注。",
        ])}
      </div>
    </section>
  `;
...
function renderPreviewStrategyNote() {
  const note = state.strategy === "version"
    ? "指定版本按已选分发范围、源版本和策略条件动态匹配设备，提交审批后执行。"
    : state.strategy === "file"
      ? "文件导入按设备清单预检，发布后由后端按设备所属大区自动拆分分发。"
      : "手动导入用于小批量设备验证，确认发布后按设备所属大区自动分发。";
  return `
    <div class="annotated-block compact-note-block">
      <div class="publish-flow-note">
        <h4>策略预览口径</h4>
        <p>${note}</p>
      </div>
      ${renderRequirementNote("PRD 03 / 15", "预览弹窗策略口径", [
        "预览发布需展示基本信息、升级策略、策略条件、升级数量和发布动作。",
        "指定版本展示已选分发范围和源版本，不展示未知总量预估。",
        "文件/手动导入展示清单预检结果和异常分类，异常中不包含区域不匹配。",
      ])}
    </div>
  `;
}
      ...
      <div class="modal-section preview-wide-section">
        ...
        <div class="info-grid preview-strategy-grid">
          <dl>
            ...
            <dt>升级区域：</dt><dd>${state.strategy === "version" ? "按已选区域" : "按设备所属大区自动分发"}</dd>
  ...
  return `
    <div class="modal-backdrop" data-action="close-modal">
      <section class="modal medium" role="dialog" aria-modal="true" aria-labelledby="endTitle" data-stop>
        <header class="modal-header"><span id="endTitle">中止任务</span><button class="modal-close" data-action="close-modal">${icon("close")}</button></header>
        <div class="modal-body">
          ${task ? `<div class="modal-task-context"><span>当前任务</span><strong>${escapeHtml(task.name)}</strong><code>${task.id}</code></div>` : ""}
          <div class="preview-alert error">${icon("alert")} 当前任务可能已有设备正在升级。确认后系统将停止新增匹配和新增下发，并进入中止中等待已下发设备回执。</div>
          <label class="form-label" style="justify-content:flex-start">中止原因</label>
          <textarea class="textarea" placeholder="请输入中止原因"></textarea>
        </div>
        <footer class="modal-footer">
          <button class="btn" data-action="close-modal">取消</button>
          <button class="btn danger" data-action="confirm-end">确认中止</button>
        </footer>
      </section>
    </div>
  `;
...
function updateMaxDispatchTotal(input) {
  ...
  if (state.maxDispatchTotal === "" || Number(state.maxDispatchTotal) > 0) {
...
function handleDeviceExport(scope) {
  state.deviceExportMenuOpen = false;
  if (scope === "all") {
    showToast("正在生成全部设备明细，完成后将触发浏览器下载");
    window.clearTimeout(handleDeviceExport.timer);
    handleDeviceExport.timer = window.setTimeout(() => {
      showToast("全部设备明细已生成，浏览器将开始下载");
    }, 1200);
    render();
    return;
  }
  const filterText = state.detailDeviceKeyword.trim() ? "当前筛选结果已生成，浏览器将开始下载" : "当前区域设备明细已生成，浏览器将开始下载";
...
function validateStep(step) {
  ...
  if (step === 2) {
    ...
    if (state.strategy === "version" && !normalizeDispatchRegions(state.dispatchRegions).length) errors.dispatchRegions = "请选择升级区域";
    if (state.strategy === "version" && !state.selectedVersions.length) errors.strategy = "请至少选择一个源版本";
    if (state.strategy === "version" && state.quantityMode === "batch" && Number(state.batchLimit) <= 0) errors.batchLimit = "请输入大于 0 的每个区域每个版本单批数";
    if (state.strategy === "version" && state.quantityMode === "batch" && state.maxDispatchTotal !== "" && Number(state.maxDispatchTotal) <= 0) errors.maxDispatchTotal = "请输入大于 0 的任务累计下发上限";
...
function handleAction(action, el) {
  switch (action) {
    ...
    case "toggle-dispatch-region": {
      ...
      showToast(`升级区域：${dispatchRegionSummary(state.dispatchRegions)}`);
      render();
      break;
    }
    case "set-top-region": {
    ...
    case "set-detail-region-tab":
      ...
      render();
      break;
    case "toggle-device-export-menu":
      state.deviceExportMenuOpen = !state.deviceExportMenuOpen;
      render();
      break;
    case "change-page":
