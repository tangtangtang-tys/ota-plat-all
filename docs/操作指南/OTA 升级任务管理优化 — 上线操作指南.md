...
| **修订时间** | **修订描述** | **修订人** | **文档版本** |
| --- | --- | --- | --- |
| 2026-06-30 | 补充上线审查缺口、高风险口径、核心页面截图，并将流程图统一调整为 UML 图源码口径 | 产品团队 | v1.1 |
...
流程图统一使用 UML 图源码描述，页面截图用于帮助运维、测试和审批人员识别真实页面控件。截图文件统一存放在 `docs/assets/screenshots/`，每张截图均需能看到对应页面的关键字段、按钮、状态和风险提示。
...
| **任务状态** | **可操作项** | **说明** |
| --- | --- | --- |
| 待发布 | 编辑、删除、提交发布 | 待发布为草稿态，可继续完善配置。 |
...
:按分发范围、源版本和策略条件动态匹配设备;
:按当前分发范围 tab 查看概览、异常、设备列表和导出;
stop
@enduml
```
...
**注意**：由于当前执行链路按分区返回设备明细，详情页不提供跨分区的统一设备明细入口。查看问题设备时，请先选择对应分区。
...
**注意**：结束任务属于高风险操作，确认后不可恢复。若需要重新升级，请在任务进入终态后使用复制重建或重新创建任务。
...
title OTA 任务状态流转
[*] --> 待发布 : 保存草稿
待发布 --> 待审批 : 指定版本/文件导入提交审批
待发布 --> 待执行 : 手动导入确认发布且未到开始时间
待发布 --> 升级中 : 手动导入确认发布且已到开始时间
待发布 --> [*] : 删除草稿
...
流程图统一使用 UML 图源码，说明业务流转和状态关系；页面截图用于展示真实页面控件、字段、按钮和弹窗。上线交付时两者都需要，不能用流程图替代页面截图。
